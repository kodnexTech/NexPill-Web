...
{{-- Revenue Banner --}}
<div class="mb-5 flex items-center gap-4 rounded-2xl bg-gradient-to-r from-[#062f2c] to-[#0a4a44] p-5 text-white shadow-sm">
...
<div class="mb-5 rounded-2xl border border-[#dde8e5] bg-white p-4 shadow-sm">
    <form method="GET" class="flex flex-wrap gap-3">
        ...
        <select name="plan_id" class="rounded-xl border border-[#dde8e5] bg-[#f6faf8] px-3 py-2.5 text-sm focus:border-[#0f806f] focus:outline-none">
        ...
        @if(request()->hasAny(['search','status','plan_id']))
            <a href="{{ route('admin.subscriptions.index') }}" class="flex items-center rounded-xl border border-[#dde8e5] px-4 py-2.5 text-sm text-[#60716e] hover:bg-[#f6faf8] transition-colors">Clear</a>
        @endif
    </form>
</div>
...
<div class="rounded-2xl border border-[#dde8e5] bg-white shadow-sm overflow-hidden">
    <div class="border-b border-[#dde8e5] px-6 py-4">
        <p class="text-sm font-semibold text-[#102a2a]">{{ $subscriptions->total() }} subscriptions</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-[#dde8e5] bg-[#f6faf8]">
                    ...
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#60716e]">Plan</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#60716e]">Status</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#60716e]">Started</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#60716e]">Ends</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#60716e]">Provider</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-[#f0f6f4]">
                @forelse($subscriptions as $sub)
                    <tr class="hover:bg-[#f9fcfb] transition-colors">
                        <td class="px-6 py-3">
                            <a href="{{ route('admin.users.show', $sub->user_id) }}" class="font-medium text-[#0f806f] hover:underline">{{ $sub->user->name ?? '—' }}</a>
                            <p class="text-xs text-[#60716e]">{{ $sub->user->email ?? '' }}</p>
                        </td>
                        <td class="px-4 py-3">
                            <p class="font-semibold text-[#102a2a]">{{ $sub->plan->name ?? '—' }}</p>
                            @if($sub->plan) <p class="text-xs text-[#60716e]">{{ number_format($sub->plan->price_minor / 100, 2) }} {{ $sub->plan->currency }}/{{ $sub->plan->billing_period }}</p> @endif
                        </td>
                        <td class="px-4 py-3">
                            @php $sc = ['active'=>'bg-[#e4f5f1] text-[#0f806f]','cancelled'=>'bg-red-100 text-red-700','expired'=>'bg-gray-100 text-gray-600','trial'=>'bg-blue-100 text-blue-700']; @endphp
                            <span class="rounded-full px-2.5 py-1 text-[11px] font-bold {{ $sc[$sub->status] ?? 'bg-gray-100 text-gray-600' }}">{{ ucfirst($sub->status) }}</span>
                        </td>
                        <td class="px-4 py-3 text-xs text-[#60716e]">{{ $sub->starts_at->format('d M Y') }}</td>
                        <td class="px-4 py-3 text-xs text-[#60716e]">{{ $sub->ends_at ? $sub->ends_at->format('d M Y') : '—' }}</td>
                        <td class="px-4 py-3 text-xs text-[#60716e]">{{ $sub->provider ?? '—' }}</td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-6 py-12 text-center text-sm text-[#60716e]">No subscriptions found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    @if($subscriptions->hasPages())
        <div class="border-t border-[#dde8e5] px-6 py-4">{{ $subscriptions->links() }}</div>
    @endif
</div>
@endsection
