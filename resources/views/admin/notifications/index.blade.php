...
<div class="mb-5 rounded-2xl border border-[#DCE6F2] bg-white p-4 shadow-sm">
    <form method="GET" class="flex flex-wrap gap-3">
        ...
        <select name="type" class="rounded-xl border border-[#DCE6F2] bg-[#F4FAFF] px-3 py-2.5 text-sm focus:border-[#075DE7] focus:outline-none">
            <option value="">All Types</option>
            @foreach($types as $type)
                <option value="{{ $type->value }}" {{ request('type') === $type->value ? 'selected' : '' }}>{{ $type->name }}</option>
            @endforeach
        </select>
        <select name="read" class="rounded-xl border border-[#DCE6F2] bg-[#F4FAFF] px-3 py-2.5 text-sm focus:border-[#075DE7] focus:outline-none">
        ...
        @if(request()->hasAny(['search','type','read']))
            <a href="{{ route('admin.notifications.index') }}" class="flex items-center rounded-xl border border-[#DCE6F2] px-4 py-2.5 text-sm text-[#5B6D86] hover:bg-[#F4FAFF] transition-colors">Clear</a>
        @endif
    </form>
</div>
...
<div class="rounded-2xl border border-[#DCE6F2] bg-white shadow-sm overflow-hidden">
    <div class="border-b border-[#DCE6F2] px-6 py-4">
        <p class="text-sm font-semibold text-[#10233F]">{{ $notifications->total() }} notifications</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-[#DCE6F2] bg-[#F4FAFF]">
                    ...
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#5B6D86]">Title</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#5B6D86]">Type</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#5B6D86]">Read</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#5B6D86]">Sent</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-[#EDF5FD]">
                @forelse($notifications as $notif)
                    <tr class="hover:bg-[#FAFCFF] transition-colors {{ !$notif->read_at ? 'bg-[#fffbf4]' : '' }}">
                        <td class="px-6 py-3">
                            <a href="{{ route('admin.users.show', $notif->user_id) }}" class="font-medium text-[#075DE7] hover:underline">{{ $notif->user->name ?? '—' }}</a>
                        </td>
                        <td class="px-4 py-3">
                            <p class="font-semibold text-[#10233F]">{{ $notif->title }}</p>
                            <p class="text-xs text-[#5B6D86] truncate max-w-60">{{ $notif->message }}</p>
                        </td>
                        <td class="px-4 py-3">
                            <span class="rounded-lg bg-[#EDF5FD] border border-[#DCE6F2] px-2 py-0.5 text-[10px] font-mono font-semibold text-[#5B6D86]">{{ $notif->type->value ?? $notif->type }}</span>
                        </td>
                        <td class="px-4 py-3">
                            @if($notif->read_at)
                                <span class="rounded-full bg-[#E5F2FF] px-2.5 py-1 text-[10px] font-bold text-[#075DE7]">Read</span>
                            @else
    ...
    @if($notifications->hasPages())
        <div class="border-t border-[#DCE6F2] px-6 py-4">{{ $notifications->links() }}</div>
    @endif
</div>
@endsection
